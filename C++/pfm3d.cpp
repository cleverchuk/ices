...
//Arrays
double params[] = {A,B,GD,GE,GM,SD,SE,VD};
double MAX[] = {MA, MB, MGD, MGE, MGM, MSD, MSE, MVD};
double upperbound[] = {uba, ubb, ubgd, ubge, ubgm, ubsd, ubse, ubvd};

double lowerbound[] = {lba,lbb,lbgd,lbge,lbgm,lbsd,lbse,lbvd};
string mapback_dict[] = {"A","B","GD","GE","GM","SD","SE","VD"};
/*
mapback_dict.push_back(a);
mapback_dict.push_back(b);
mapback_dict.push_back(gd);
mapback_dict.push_back(ge);
mapback_dict.push_back(gm);
mapback_dict.push_back(sd);
mapback_dict.push_back(se);
mapback_dict.push_back(vd);
*/
int current;
...
// Should have name it init
// but resetRho came out of circumstances
// you can rename at your own headache
// what it does is pretty much what its name says with other stuff
// don't mess with, You've been warned!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	srand(0);
 static int count = 0;
...
// con: is a pointer to the struct of the constants
// count: count is used to properly retrieve the variabel arguments
// for the ellipses => you already know; if you don't Google it
	va_list args;	
	va_start(args, count);

	for(int i = 0; i < count; i++){
		int index = va_arg(args,int);
		string param = mapback_dict[index];
//double params[] = {A,B,GD,GE,GM,SD,SE,VD};

		if( param.compare( mapback_dict[0]) == 0)
			cons->A = params[index];

		else if( param.compare( mapback_dict[1]) == 0)
			cons->B = params[index];
		
		else if( param.compare( mapback_dict[2]) == 0)
			cons->gbdif = params[index];

		else if( param.compare( mapback_dict[3]) == 0)
			cons->gbenergy = params[index];

		else if( param.compare( mapback_dict[4]) == 0)
			cons->gbmobility = params[index];

		else if( param.compare( mapback_dict[5]) == 0)
			cons->surfdif = params[index];

		else if( param.compare( mapback_dict[6]) == 0)
			cons->surfenergy = params[index];

		else if( param.compare( mapback_dict[7]) == 0)
...
// master determines which method call does the switch and resets the time because resetting the time is critical for the simulation
// every other parameter is self explanatory
// returns a boolean so can either restart or continue with current values of your constants
static int count = 0;
static bool isWritable = true;
/*			cout <<"time : " << *t <<endl;	
			cout << "param:" << *param << " rho: "<< *sumrho <<endl;
			cout << "maximum:" << *maximum << *sumrho <<endl;
			cout <<"lowerbound: " << *lowerbound << " upperbound: " << *upperbound <<endl;
*/		if(*upperbound <= 0 || *upperbound > *maximum)
			*upperbound = *maximum;
		...
		if(isnan((*sumrho)) || *sumrho < 0 || *sumrho > RHO){
			...
			*param = inRange(*lowerbound, *upperbound, *t);
			if( count > 5){
				*upperbound -= 0.1;
				*lowerbound = 0;
...
template<typename T> void clear_1D(T* ptr){
	delete[] ptr;
}
template<typename T> void clear_2D(T** ptr, int size){
...
static bool writeonce = true; // making sure that combination heading is written only once
...
SwitchRho = rho; rho = NewRho; NewRho = SwitchRho;
...
*/
/*	cout << "con.A = " << con.A << ";\n" <<"con.B = " << con.B << ";\n" <<"con.gbdif = " << con.gbdif << ";\n" <<"con.gbenergy = " << con.gbenergy <<endl;
	cout << "con.gbmobility = " << con.gbmobility << ";\n" <<"con.surfdif = " << con.surfdif << ";\n" <<"con.surfenergy = " << con.surfenergy <<endl;
	cout <<	"con.voldif = " << con.voldif << endl;
	cout << "rho = " << sumrho <<endl;
	cout<<"========================="<<endl;
*/
ofstream fout("Combinations.txt",ios::out | ios::app);
switch(current){
